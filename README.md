...
Focused acceptance commands belong to implementation and review workers. Final verification alone owns the lifecycle-wide suite, records its exit status on the exact verified commit, and executes it at most once per commit; matching passing evidence is reused and matching failures are reported without rerun. Documentation findings always pass through review, address, and re-review before verification.
