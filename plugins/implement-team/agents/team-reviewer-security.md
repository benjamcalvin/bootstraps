...
`SendMessage` is a narrow tool, not a general collaboration channel. Use it only when you cannot answer the question by reading the code — for example, whether an upstream layer already enforces authorization or whether a validation gap depends on invariants that aren't written down. Before sending, re-read the relevant file; if the answer is there, don't ask.
